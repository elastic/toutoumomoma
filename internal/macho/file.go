...
import (
	...
	"github.com/elastic/toutoumomoma/internal/saferio"
)
...
func NewFile(r io.ReaderAt) (*File, error) {
	...
	for i := 0; i < int(f.Ncmd); i++ {
		...
		case LoadCmdDysymtab:
			...
			switch {
			case hdr.Iundefsym > uint32(len(f.Symtab.Syms)):
			...
			case hdr.Iundefsym+hdr.Nundefsym < hdr.Iundefsym:
				return nil, &FormatError{offset, fmt.Sprintf(
					"number of undefined symbols after index in dynamic symbol table command overflows (%d + %d)",
					hdr.Iundefsym, hdr.Nundefsym), nil}
			case hdr.Iundefsym+hdr.Nundefsym > uint32(len(f.Symtab.Syms)):
				return nil, &FormatError{offset, fmt.Sprintf(
